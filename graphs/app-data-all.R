...
data.mobile1 = read.table ("/home/jelfn/git/icc/results/ICCScenario/normal/5/app-delays", header=T)
data.mobile2 = read.table ("/home/jelfn/git/icc/results/ICCScenario/normal/no-inf/app-delays", header=T)
...
mobile1name = "NNN Inf with NDN Smart Flooding"
mobile2name = "NDN Smart Flooding"
...
g.all <- ggplot (satTime, aes(color=Variable)) +
  geom_line (aes (x=TimeSec, y=DelayS.mean), size=1) +
  ggtitle ("Average Delay For NDN Application") +
  ylab ("Delay [Seconds]") +
  xlab ("Simulation Time [Seconds]") +
  scale_colour_discrete(name = "Forwarding strategy",
...
g.all <- ggplot (satTime, aes(color=Variable)) +
  ...
  ggtitle ("Average Packet Hop Count For NDN Application") +
  ylab ("Hop Count") +
  xlab ("Simulation Time [Seconds]") +
