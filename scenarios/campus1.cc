...
int main (int argc, char *argv[])
{
	...
	int nCN = 1, nLANClients = 10;//  trial
	bool nix = true;
...
//	p2p_2gb200ms.SetDeviceAttribute ("DataRate", StringValue ("2Gbps"));	//for ring
...
//	  	consumerHelper1.SetAttribute ("Frequency", DoubleValue (intFreq));
...
//	  	consumerHelper1.SetAttribute ("NumberOfContents", UintegerValue(NumberOfContents));
		...
		for(int i =0; i<7; i++){
			for (int j=0; j <nLANClients; j++){
				...
				clientNodes.Add(nodes_net2LAN[0][i][j]);
//				cout << "Consumer : " << nodes_net2LAN[0][i][j].g << endl;
			}
		}
		for(int i =0; i<5; i++){
					for (int j=0; j <nLANClients; j++){
						consumerHelper1.Install (nodes_net3LAN[0][i][j]);
						clientNodes.Add(nodes_net3LAN[0][i][j]);
					}
				}
...
//	ndnGlobalRoutingHelper.AddOrigins ("/OD/CD", nodes_net1[0][0].Get (0)); //trial
	...
	// NDN L3 tracer
	sprintf (filename, "%s/%dcampus-rate-trace", results, clients);
//	ndn::L3RateTracer::InstallAll (filename, Seconds (1.0));
	ndn::L3RateTracer::Install(odNodes, filename, Seconds (1.0));
	sprintf (filename, "%s/%d-client-campus-rate-trace", results, clients);
	ndn::L3RateTracer::Install(clientNodes, filename, Seconds (1.0));

	// NDN App Tracer
	sprintf (filename, "%s/campus-app-delays", results);
