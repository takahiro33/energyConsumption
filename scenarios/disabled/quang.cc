...
int main (int argc, char *argv[])
{
	...
	uint32_t NumberOfContents = 100;			// Number of Contents
	...
	int contentSize = 100;                       // Size of content to be retrieved
	...
	p2p_100mbps5ms.SetDeviceAttribute ("DataRate", StringValue ("1000Mbps"));
...
//	ndn::AppHelper consumerHelper ("ns3::ndn::ConsumerCbr");
	...
	if (traceFiles) {
		...
		char mode[7];
		if(csSize > 0) sprintf(mode, "ccn");
		else sprintf(mode, "ip");
		sprintf(results, "%s/%s/MN:%d", results, mode, mobile);
