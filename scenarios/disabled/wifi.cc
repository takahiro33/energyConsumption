...
int main (int argc, char *argv[])
{
	...
	uint32_t trainAp = 1;			      		// Number of train wagons
...
*/
	...
	for(int i = 1; i <= users+trainAp; i++)
	{
		if(i < users){
			mobileXpos.push_back(100 * i / users);
			mobileYpos.push_back(100);
		} else {
			mobileXpos.push_back(100 * i / trainAp);
...
//	int intFreq = int((MBps * 1000000) / payLoadsize);
	...
	for (int i = 0; i < users+trainAp; i++)
	{
		if(i < users) 	mobileNodeIds.push_back(userNodes.Get (i)->GetId ());
		else 			mobileNodeIds.push_back(trainApNodes.Get (i)->GetId ());
	}
	...
	NetDeviceContainer wifiMTNetDevices = wifi.Install (wifiPhyHelper, wifiMacHelper, userNodes);
	 //wifiMTNetDevices = wifi.Install (wifiPhyHelper, wifiMacHelper, trainApNodes);
	...
	// Create a NDN stack for the clients and users node
	...
	ndnHelperUsers.Install (userNodes);
...
//		ndn::CsTracer::InstallAll (filename, Seconds (1));
	...
	for (int i =0; i < trainAp; i++){
		// This causes the device in mtId to change the SSID, forcing AP change
		uint32_t mtId = trainApNodes.Get (i)->GetId();
			sprintf(configbuf, "/NodeList/%d/DeviceList/0/$ns3::WifiNetDevice/Mac/Ssid", mtId);
